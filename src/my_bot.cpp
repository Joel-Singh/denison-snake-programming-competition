...
Direction MyBot::think(const Grid &grid) const {
  ...
  if(grid.get_current_tick()>5){
    ...
    if((grid.get_current_tick()<150) && (fruitToDist<evilTotDist) && (evilTotDist>2)){
...
std::vector<Direction> getAvalDir(Pos head, const Grid &grid){
  ...
  if(is_safe_to_move(head.with_dir(UP), grid)){
    ...
    Pos head2 = head;
    head2.y = head2.y+1;
    if(is_safe_to_move(head2.with_dir(UP), grid) || is_safe_to_move(head2.with_dir(DOWN), grid) || is_safe_to_move(head2.with_dir(LEFT), grid) || is_safe_to_move(head2.with_dir(RIGHT), grid)){
...
Pos nearstFruit(std::vector<Pos> fruits, Pos head){
  ...
  for(int i=0; i<fruits.size(); i++){
    int fruitDist = getDist(head,closestFruit).x + getDist(head,closestFruit).y;
