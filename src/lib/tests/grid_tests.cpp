...
TEST(Grid, get_methods) {
  ...
  Grid grid(false, 18, cells, std::vector<Pos>(), std::vector<Pos>());

  EXPECT_EQ(grid.get_height(), 10);
  EXPECT_EQ(grid.get_width(), 10);
  EXPECT_EQ(grid.get_current_tick(), 18);
...
TEST(Grid, find_methods) {
  ...
  Grid grid(true, 0, cells, player_one_segments, player_two_segments);
